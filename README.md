...
- **桌面挂件**：今日待办列表，完成 / 删除（确认 + 撤回）/ 独立窗口编辑标题
- **紧急评分**：1–5 星，影响列表排序
- **标签**：工作/生活/学习互斥，可叠加「紧急」与自定义标签；支持按标签筛选（重启后恢复）
- **步骤清单**：待办下可边做边加步骤，展示用时/已进行天数
- **等待中**：可标记等待并填写原因，展示已等待天数；结束等待后保留历史，可在右键面板查看全部等待记录
- **右键面板**：添加时间与已过天数、预计完成天数、标签、步骤、等待
- **快捷添加**：全局快捷键唤起小窗，标题可换行；失焦保持打开，Enter 即可添加
- **日切滚动**：跨天后，进行中/等待中的待办自动归属到今天
- **完成日历**：按月回看每日完成情况，支持恢复为进行中
- **主题与透明度**：默认深色、透明度 75%；可切换浅色并调节透明度
- **显示模式**：普通窗口，或固定到桌面（壁纸软件 / 系统壁纸，Win+D 后仍可能可见）
- **挂件透明度**：0–100%，默认 75%
- **始终置顶**：可选悬浮在任意页面之上
- **系统托盘**：点击显示挂件，右键退出
- **开机自启**、快捷键自定义
...
| 类型 | 文件名示例 | 说明 |
|------|------------|------|
| **便携版（推荐）** | `TODO-Portable-x.y.z.exe` | 主推：单文件绿色运行，数据在 exe 旁 `data/`，更新也走便携 channel |
| 安装版 | `TODO-Setup-x.y.z.exe` | 备选：NSIS 安装包，可改安装目录，数据在 AppData |
...
> **提示：** 开发调试时请先退出正在运行的便携版/安装版，避免全局快捷键与桌面附着互相抢占。
...
瘦身后安装包约 82MB，可上传 Gitee（社区版附件上限 100MB）。
